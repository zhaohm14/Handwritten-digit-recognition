function Answer=unit_inputdlg(Prompt, Title, NumLines, DefAns, Resize)
...
error(nargoutchk(0,1,nargout)); %#ok
...
FunctionDefineHandle = uicontrol(InputFig , ...
    BtnInfo         , ...
    'Position'      , [FigWidth - 3 * BtnWidth - 3 * DefOffset, DefOffset, BtnWidth, BtnHeight], ...
    'KeyPressFcn'   , @doFcnKeyPress, ...
    'String'        , 'Use Function', ...
    'Callback'      , @FcnDefineCallback, ...
    'Tag'           , 'FcnDefine' ...
    ); %#ok
...
function doFcnKeyPress(obj, evd) %#ok
switch(evd.Key)
    case {'return', 'space'}
         FcnDefineCallback;
    case {'escape'}
...
function FcnDefineCallback(obj,evd)
global NumQuest;
global EditHandle;
UnitFcn = char(FcnDefine);
if ~isempty(UnitFcn)
    for ii = 1 : NumQuest
        Unit_ii = eval(UnitFcn);
        set(EditHandle(ii), 'String', Unit_ii);
    end
end